...
function codubigeo($con,$iddis){
    if($con!="" && $iddis!=""){
        $iddis=iseguro($con,$iddis);
        $cu=mysqli_query($con,"SELECT di.ubigeo dis, pr.ubigeo pro, de.ubigeo dep FROM distrito di INNER JOIN provincia pr ON di.idprovincia=pr.idprovincia INNER JOIN departamento de ON pr.iddepartamento=de.iddepartamento WHERE di.iddistrito=$iddis;");
        if($ru=mysqli_fetch_assoc($cu)){
            return $ru['dep'].$ru['pro'].$ru['dis'];
        }else{
            return "-";
        }
        mysqli_free_result($cu);
